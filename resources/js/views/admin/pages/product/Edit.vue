							...
							if (res.flag) {
								if (["category"].includes(value)) {
									res.data.forEach(item =>
										this[value].list.push(item.pivot[`${value}_id`])
									);
									this[value].default = this[value].list;
								} else {
									res.data.forEach(
										item => (this.form[`${value}_id`] = item.id)
									);
								}
							}
						});
				...
				if (valid) {
					...
					if (!this.isEdit) {
						...
						this.createItem(data).then(res => {
							if (res.flag) this.$fire(foo.NOTIFICATION.success.created);
							else {
						...
						this.updateItem(data).then(res => {
							if (res.flag) this.$fire(foo.NOTIFICATION.success.updated);
							else {
								let errors = res.msg.response.data.errors;
								this._showErrors(errors);
							}
