        ...
        <div class="col-lg-3 col-sm-6" v-for="item in items" :key="item.id">
          <div class="item-product">
            <a href title class="img-primary">
              <img :src="_getThumbnail('product', item.thumbnail)" alt />
            </a>
            <div class="ct">
              <h3 class="title">
                <router-link
              ...
              <div class="price">
                ...
                <del
                  v-show="item.original_price"
                >{{ _formatCurrency(item.original_price) }}{{ `/${_notag(item.unit.title)}` }}</del>
              </div>
            </div>
            <span class="sales" v-show="getSale(item) > 0">-{{ getSale(item) }}%</span>
