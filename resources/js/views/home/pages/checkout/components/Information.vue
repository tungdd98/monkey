				...
				<el-row :gutter="20">
					...
					<el-col :span="8">
						<div class="Yorder">
							<table class="table table-bordered">
								...
								<tbody v-if="cart.all.length > 0">
									<tr v-for="item in cart.all" :key="item.product.id">
										...
										<td class="text-right">{{ _formatCurrency(item.quantity * item.product.price) }}</td>
									</tr>
								</tbody>
								<tr>
									<td>Tạm tính</td>
							...
							<button @click.prevent="handleSaveInformation">Tiếp tục</button>
						</div>
					</el-col>
				</el-row>
			</div>
	...
	methods: {
		handleSaveInformation() {
			this.$refs.information.validate(valid => {
				if(valid) {
					this.$store.dispatch("cart/addCustomer", this.form).then(res => {
						if(res.flag) {
