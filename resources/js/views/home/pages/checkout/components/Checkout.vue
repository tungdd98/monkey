<template>
	<div class="page-checkout">
		<router-link v-if="!user" :to="{ name: 'information' }" class="back-btn"><i class="el-icon-back"></i> Quay lại</router-link>
		<div class="card">
			...
			<div class="card-body checkout">
				...
				<div class="checkout__customer" v-if="user || customer">
					...
					<div class="row mb-2">
						...
						<div class="col-md-9">
							<span>{{ user ? _notag(user.name) : _notag(customer.name) }}</span>
						</div>
					</div>
					<div class="row mb-2">
						<div class="col-md-3">
						...
						<div class="col-md-9">
							<span>{{ user ? _notag(user.address) : _notag(customer.address) }}</span>
						</div>
					</div>
					<div class="row mb-2">
						<div class="col-md-3">
						...
						<div class="col-md-9">
							<span>{{ user ? _notag(user.email) : _notag(customer.email) }}</span>
						</div>
					</div>
					<div class="row mb-2">
						<div class="col-md-3">
						...
						<div class="col-md-9">
							<span>{{ user ? _notag(user.phone) : _notag(customer.phone) }}</span>
						</div>
					</div>
				</div>
	...
	methods: {
		...
		handleSubmit() {
			...
			if(this.customer) {
				...
				Object.entries(this.customer).forEach(([key, value]) => data.append(key, value))
			}
