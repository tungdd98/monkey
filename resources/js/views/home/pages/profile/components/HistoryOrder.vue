			...
			<table class="table">
				...
				<tbody v-if="data">
					<tr v-for="item in data" :key="item.id" class="text-center">
						<td>
							<a href @click.prevent="handleShow(item)">{{ 'KR' + item.id }}</a>
						</td>
						<td>{{ _dateFormat(item.created_at, 'short') }}</td>
						<td>{{ item.quantity }}</td>
						<td>{{ _formatCurrency(item.total) }}</td>
						<td><div class="v-h2" v-html="handleShowStatus(item)">{{ handleShowStatus(item) }}</div></td>
					</tr>
				</tbody>
			</table>
		</div>
		<div class="order-detail" v-if="currItem">
			<el-dialog :title="display.formTitle" :visible.sync="dialog.formVisible">
				<div class="d-flex justify-content-end mb-2" v-if="currItem.status === 0">
					<div class="btn btn-outline-danger btn-sm" @click.prevent="handleChangeStatus">Huỷ đơn hàng</div>
				</div>
				<div class="d-flex align-items-center justify-content-between">
					...
					<div>
						<p class="v-h2" style="font-size: 24px">{{ _formatCurrency(currItem.total) }}</p>
					</div>
				</div>
				<hr />
				<div class="row kr-info">
					<div class="col-md-6 kr-info__customer">
						...
						<div class="kr-info__box">
							<p class="v-h2" style="text-transform: uppercase">{{ _notag(currItem.customer.name) }}</p>
							<p>Địa chỉ: {{ _notag(currItem.customer.address) }}</p>
							<p>Số điện thoại: {{ _notag(currItem.customer.phone) }}</p>
						</div>
					</div>
					<div class="col-md-6 kr-info__pay">
						...
						<div class="kr-info__box">
							...
							<p>(Tomita Mart sẽ đến thu tiền và giao sản phẩm tại địa chỉ của quý khách.)</p>
				...
				<div class="md-cart-tb mt-4">
					<table>
						...
						<tbody>
							<tr v-for="item in currItem.products" :key="item.id">
								<td>
									<div class="if-pro-cart">
										<a class="img" href="#" title>
											<img :src="_getThumbnail('product', item.thumbnail)" alt title />
										</a>
										<div class="ct">
											<span>{{ _notag(item.title) }}</span>
										</div>
									</div>
								</td>
								<td>
									<strong>{{ item.original_price ? _formatCurrency(item.price) : _formatCurrency(item.original_price) }}</strong>
									<del v-if="item.original_price">{{ _formatCurrency(item.original_price) }}</del>
								</td>
								<td>{{ item.pivot.quantity }}</td>
								<td>{{ _formatCurrency(getToTalMoneyItem(item.pivot.quantity, item.price)) }}</td>
							</tr>
						</tbody>
					</table>
				</div>
				<div class="md-cart-foot">
					<div class="top">
						<span class="total-provision">Tổng tạm tính: {{ _formatCurrency(getTotalMoney) }}</span>
						...
						<div class="total">
							<p>
								Tổng cộng:
								<strong>{{ _formatCurrency(getTotalMoney) }}</strong>
							</p>
...
export default {
	...
	computed: {
		 ...
		 */
		showStatus() {
			return `<span style="color: ${foo.ORDER[this.currItem.status].color}">${
				foo.ORDER[this.currItem.status].label
			}</span>`;
    },
    getTotalMoney() {
	...
	methods: {
      ...
      return `<span style="color: ${foo.ORDER[item.status].color}">${foo.ORDER[item.status].label}</span>`
		},
		/**
		 * Hiển thị sản phẩm
		 */
